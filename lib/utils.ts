...
export class Utils {
	...
	static buildAuthorIdentity(payload: {
		...
		const signature = Utils.signArguments(
			Object.assign({}, payload, { args: payloadWithOpReturn, indexes })
		);
		const constructed = [
	 ...
	 */
	static validateAuthorIdentityOccurenceAtPos(
		args: any[],
		pos: number
	): {
		valid: boolean;
		...
		if (
			!args[pos + algorithmSchemePos] ||
			Buffer.from(args[pos + algorithmSchemePos], 'hex').toString() !== 'BITCOIN_ECDSA'
		) {
	...
	static verifyAuthorIdentity(
		...
		for (let scanPrefixCounter = 0; scanPrefixCounter < cleanedArgs.length; scanPrefixCounter++) {
			...
			if (authorIdentityPrefix == decodedField) {
				...
				if (
					verificationResult.addresses[expectingSignatureIndex] &&
					...
					verificationResult.addresses[expectingSignatureIndex].fieldIndexesForSignature =
						result.fieldIndexesForSignature;

					if (
						verificationResult.signedFullyByAddresses &&
	...
	static areAllFieldsIncludedUpto(
